...
from auxiliary.adaptive_control.contracts import AdaptiveReadinessSnapshot, ControlProfile, json_bytes
from auxiliary.adaptive_control.models import RealtimeMultimodalWindowAdapter
from auxiliary.adaptive_control.physio_monitor import build_physio_snapshot, downsample_values, summarize_values
from auxiliary.adaptive_control.policy import ControlEstimate, AdaptiveControlPolicy
...
def test_adaptive_control_is_not_exposed_by_the_thesis_cli():
    parser = build_parser()
    choices = next(action.choices for action in parser._actions if getattr(action, "choices", None) is not None)

    assert "adaptive-control" not in choices
    assert "adaptive-model" not in choices
...
def test_launcher_uses_configured_default_bundle():
    ...
    settings = yaml.safe_load((root / "auxiliary" / "adaptive_control" / "configs" / "adaptive-control.yaml").read_text(encoding="utf-8"))
    launcher = (root / "auxiliary" / "adaptive_control" / "start-adaptive-control.ps1").read_text(encoding="utf-8")

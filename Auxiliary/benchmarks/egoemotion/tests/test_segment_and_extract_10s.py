...
from Auxiliary.benchmarks.egoemotion.scripts.segment_and_extract_10s import resolve_encoder_dirs
from mac.encoders.registry import ModalityRegistry


def test_10s_extraction_writes_manifest_hash_to_embedding_payloads():
    text = Path(
        "Auxiliary/benchmarks/egoemotion/scripts/segment_and_extract_10s.py"
    ).read_text()
